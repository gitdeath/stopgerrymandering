...
def build_adjacency_graph(gdf, progress_interval: int = 5000):
    ...
    Edges: blocks that 'touch' (share boundary/vertex).
    ...
    # --- Strategy selection -------------------------------------------------
    # Try the fastest route first (bulk with predicate='touches'); else try bulk bbox;
    ...
    if has_query_bulk:
        try:
            ...
            _ = sindex.query_bulk(gdf.geometry[:1], predicate="touches")
    ...
    if has_query_bulk and supports_predicate_bulk:
        ...
        pairs = sindex.query_bulk(gdf.geometry, predicate="touches")
        n_pairs = pairs.shape[1]
        logging.info(f"Spatial candidates (touches via index): {n_pairs:,} pairs")
        ...
        # Bulk bbox hits, then post-check touches()
        pairs = sindex.query_bulk(gdf.geometry)
        n_pairs = pairs.shape[1]
        logging.info(f"Spatial candidates (bbox): {n_pairs:,} pairs; testing touches...")
        ...
        try:
            _ = sindex.query(gdf.geometry.iloc[0:1], predicate="touches")
        ...
        for i, gi in enumerate(geoms):
            ...
            try:
                if supports_predicate:
                    candidates = sindex.query(gdf.geometry.iloc[i:i+1], predicate="touches")
                else:
                ...
                for j in getattr(candidates, "tolist", lambda: list(candidates))():
                    ...
                    if not gj or getattr(gj, "is_empty", False):
                        continue
                    if supports_predicate:
                        # already 'touches' by predicate
                        G.add_edge(geoids[i], geoids[j])
                        edges_added += 1
                    else:
                        # bbox-hit: post-check touches
                        try:
                            if gi.touches(gj):
                                G.add_edge(geoids[i], geoids[j])
                                edges_added += 1
